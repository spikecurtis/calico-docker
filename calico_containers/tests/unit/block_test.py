...
class TestAllocationBlock(unittest.TestCase):
    def test_init_block_id(self):
        ...
        assert_equal(block.get_block_id(), str(network))
...
class TestGetBlockIDForAddress(unittest.TestCase):

    @parameterized.expand([
        (IPAddress("192.168.3.7"), "192.168.3.0/24"),
        (IPAddress("10.34.11.75"), "10.34.11.0/24"),
        (IPAddress("2001:abee:beef::1234"), "2001:abee:beef::1200/120"),
        (IPAddress("2001:abee:beef::"), "2001:abee:beef::/120"),
    ])
    def test_get_block_id(self, address, id):
        """
        Test get_block_id
        """
        block_id = get_block_id_for_address(address)
        assert_equal(block_id, id)
